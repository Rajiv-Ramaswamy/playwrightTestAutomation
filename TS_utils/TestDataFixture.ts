...
// Here we are importing @playwright/test because we are extending the capabilities of the test object
// This doesn't need curly braces like { base } because we are keeping it as a variable only for now, as we are going to extend it.

interface CartCheckTestData {
...
export const customTest = baseTest.extend<{cartCheckTestData: CartCheckTestData, End_to_End_Testdata: EndToEndTestdata}> ({
